...
<div class="card m-6">
    <div class="card-body">
        ...
        <div class="table-responsive">
            ...
            @if(empty($search))
            {{ $laporan_penjualan->appends(['dari' => $tanggal_awal, 'sampai' => $tanggal_akhir])->links() }}
            @else
            {{ $laporan_penjualan->appends(['dari' => $tanggal_awal, 'sampai' => $tanggal_akhir, 'search' => $search])->links() }}
            @endif
        </div>
        @endif
    </div>
